...
Piwaku is a fast, native macOS app for working with local coding agents. It is
built in Rust with [GPUI](https://github.com/zed-industries/zed/tree/main/crates/gpui)
and keeps projects, sessions, and transcripts on your machine.

## Install

For the current release target, macOS only. Download the latest `.dmg` from
[Piwaku Releases](https://github.com/anamaxlec/Piwaku/releases).
...
Piwaku. It detects available CLIs automatically and uses each provider's native
...
- Store app state locally, with no Piwaku account or remote service required.
...
When connected to a daemon managed outside the desktop process, Piwaku never
...
replace the daemon without relaunching Piwaku Debug.

## Development

Development for this fork currently targets macOS and requires
...
Piwaku is licensed under the [GNU General Public License v3.0 only](LICENSE).
